...
var pool = module.exports = function pool(options, authorizeFn, logger) {
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function () {
                    ...
                    CheckBlockAccepted(shareData, blockHex, this._daemon, function (isAccepted, tx, height, value) {
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            options.testnet = false; //rpcResults.getinfo.testnet;
            options.protocolVersion = 70016;//rpcResults.getinfo.protocolversion;
...
console.log("*** Trying gettransaction ***");
                    ...
                    _this.daemon.cmd('gettransaction',
                        [txId],
                        function (results) {

                            if (results[0].response && results[0].response.txid == txId)
                            {
                                var newHash = results[0].response.blockhash;

console.log("OK: New hash = " + newHash);
