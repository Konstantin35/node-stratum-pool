            ...
            function(results){
                for (var i = 0; i < results.length; i++){
                    ...
                    if (result.error) {
                        ...
                        emitErrorLog('Daemon instance ' + result.instance.index + ' rejected a supposedly valid block' + JSON.stringify(result));
                        return;
                    }
                }
...
emitLog('test1');
emitLog(blockHex);
emitLog(JSON.stringify(shareData));
                    CheckBlockAccepted(shareData, shareData.blockHash, function(isAccepted, tx, newBlockHash){
                        isValidBlock = isAccepted;
                        shareData.txHash = tx;
                        shareData.blockHash = newBlockHash;
                        
emitLog('test2');
emitLog(isValidBlock);
emitLog(tx);
    ...
    function CheckBlockAccepted(shareData, blockHash, callback){
        
        // Added by MikeO
        var blockHash = shareData.blockHash;
        var txHash = shareData.txHash;
        var blockHeight = shareData.height;
        
        if (txHash)
        {
            var txId = txHash.match(/.{1,2}/g).reverse().join('');
        }
        else
        {
            var txId = false;
        ...
        _this.daemon.cmd('getblock',
            ...
            function(results){
                ...
                if (!results[0].response && txId)
                {
                
                    _this.daemon.cmd('getbestblockhash',
                        [],
                        function(resultbh){
                
   
                            var newHash = resultbh.response;

emitLog("OK: New hash = " + resultbh.response);
                            ...
                            _this.daemon.cmd('getblock',
                                ...
                                function (resultstwo) {
                                    ...
                                    let validResults = resultstwo.filter(function (result) {
                                        return result.response && (result.response.hash === newHash)
                                    });
                                
emitLog(validResults);
...
emitLog('test3');
                                        ...
                                        if (validResults[0].response.tx[0] == txId && validResults[0].response.height == blockHeight)
                                        {
emitLog('test4');
                                            
                                            //callback(true, validResults[0].response.tx[0], validResults[0].response.height);
                                            callback(true, validResults[0].response.tx[0], newHash);
                                        }
                                    }
                                    else {
                                        callback(false);
                ...
                else
                {
                    ...
                    if (validResults.length >= 1){
                        callback(true, validResults[0].response.tx[0], blockHash);
                    }
                    else{
                        callback(false);
                    }
