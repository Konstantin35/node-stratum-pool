 ...
 * @constructor
 */
function DaemonInterface(daemons) {
    ...
    function isOnline(callback) {
        cmd('getwalletinfo', [], function (results) {
    ...
    function performHttpRequest(instance, jsonData, callback) {
        ...
        var parseJson = function (res, data) {
            ...
            if (res.statusCode === 401) {
                console.log('Unauthorized RPC access - invalid RPC username or password');
                return;
            }

            if (res.statusCode !== 200 && res.statusCode !== 500) {
                console.log('Daemon returned unexpected return code %s, data = %s', res.statusCode, JSON.stringify(data));
                return;
            }
